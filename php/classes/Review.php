...
//		//build and array of reviews
	...
	public static function getReviewByReviewProfileId(\PDO $pdo, $reviewProfileId): \SplFixedArray {
		...
		//create query template
		$query = "SELECT reviewId, reviewProfileId, reviewRecAreaId, reviewContent, reviewDateTime,reviewRating, recAreaId, recAreaName FROM review INNER JOIN recArea ON review.reviewRecAreaId = recArea.recAreaId WHERE reviewProfileId = :reviewProfileId";
		$statement = $pdo->prepare($query);
		//bind the review id to the place holder in the template
