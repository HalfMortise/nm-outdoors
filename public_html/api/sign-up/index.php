...
try {
	...
	if($method === "POST") {
		...
		$profile = new Profile(generateUuidV4(), $profileActivationToken, $requestObject->profileEmail, $hash, $requestObject->profileImageUrl, $requestObject->profileUserName);

		//insert the profile into the database
		$profile->insert($pdo);

		//compose the email message to send with the activation token
		$messageSubject = "NM-Outdoors 🌄 Account Activation";
...
EOF;
		...
		// update reply
		$reply->message = "Thank you for creating a profile with NM-Outdoors! 😊 Please check your email to confirm your account. 📬";
	} else {
